...
with tab1:
    ...
    def difference(row):
        diff_density = min(abs(row['density'] - target_density) / max(target_density, 0.1), 1.0)
        diff_yield_strength = min(abs(row['yield_strength'] - target_yield_strength) / max(target_yield_strength, 1.0), 1.0)
        diff_max_service_temp = min(abs(row['max_service_temp'] - target_max_service_temp) / max(target_max_service_temp, 1.0), 1.0)
    ...
    if results1.empty:
        ...
        display_cols = ['name', 'family', 'density', 'yield_strength', 'ultimate_tensile_strength',
...
with tab2:
    ...
    if filtered_df.empty:
        st.warning("No materials match your requirements. Try relaxing your constraints.")
    else:
        ...
        display_cols2 = ['name', 'family', 'density', 'yield_strength', 'ultimate_tensile_strength',
                         'elastic_modulus', 'thermal_conductivity', 'max_service_temp',
